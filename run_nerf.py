...
from color_map import colorize
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    if savedir is not None:
        # saves the camera extrinsics
        cam_ex_filename = os.path.join(savedir, 'extrinsics')
        np.save(cam_ex_filename, render_poses.cpu().numpy())

        # saves the camera intrinsics
        cam_in_filename = os.path.join(savedir, 'intrinsics')
        np.save(cam_in_filename, K)
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        # renders the color mapped depth images
        disp_cpu = disp.cpu().numpy()
        min = np.min(disp_cpu)
        max = np.max(disp_cpu)
        disps.append(colorize(disp_cpu, min, max))

        # renders the regular depth images
        ...
        if savedir is not None:
            # saves the rgba images
            rgb8 = to8b(rgbs[-1])
            filename = os.path.join(savedir, 'rgb{:03d}.png'.format(i))
            imageio.imwrite(filename, rgb8)

            # saves the depth images
            disp_img = to8b(disps[-1])
            disp_filename = os.path.join(savedir, 'disp{:03d}.png'.format(i))
            imageio.imwrite(disp_filename, disp_img)
...
def train():
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            testsavedir = os.path.join(basedir, expname, 'renderonly2_{}_{:06d}'.format(
                'test' if args.render_test else 'path', start))
            os.makedirs(testsavedir, exist_ok=True)
            print('test poses shape', render_poses.shape)

            rgbs, disp = render_path(render_poses, hwf, K, args.chunk, render_kwargs_test,
                                     gt_imgs=images, savedir=testsavedir, render_factor=args.render_factor)
            print('Done rendering', testsavedir)
            imageio.mimwrite(os.path.join(
                testsavedir, 'video_rgb.mp4'), to8b(rgbs), fps=60, quality=8)
            imageio.mimwrite(os.path.join(testsavedir, 'video_disp.mp4'), to8b(
                disp / (np.max(disp) / 2)), fps=60, quality=8)
